...
def make_head_look_at_target(robot_parts, robot_node, target_node):
    ...
    HEAD_Z_OFFSET = 0.8  # meters above base

    # Head position (approximate, assuming fixed offset above base)
    head_pos = [
    base_pos[0],
    base_pos[1],
    base_pos[2] + HEAD_Z_OFFSET
]

    #  Extract body yaw from axis-angle
    ...
    real_yaw   = normalize_angle(world_yaw + body_yaw)  # yaw relative to body orientation
    ...
    # Clamp to limits so joints don't try to exceed them
