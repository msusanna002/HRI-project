...
def robot_to_world(robot_xyz, robot_node):
    ...
    # Position of point in robot base frame
    p_B = np.array(robot_xyz)

    # Express that vector in world frame
    p_rel_W = R_WB.dot(p_B)

    # Combine base position and relative position to get world position
    p_W = base_pos + p_rel_W
