...
GAZE_SERVER_URL = "http://127.0.0.1:8000/evaluate_move"  # make sure this matches gaze_server
...
class GameManager:
    ...
    def LLM_get_next_piece(self):
        ...
        try:
            ...
            # NOTE: start llm_bridge server separately if using HTTP (below we provide llm_agent.py)
    ...
    def gather_gaze_data(self):
        ...
        try:
            ...
            # when in doubt, conservative default: Negative (so we ask LLM)
            return False
    ...
    def step(self):
        ...
        if self.current_arm_task is not None:
            ...
            return  # IMPORTANT: stop step() here
        if self.state == "choose_next_piece":
            ...
            self.current_arm_task = MovePieceTask(self.current_piece,self.current_piece.getField("targetPos").getSFVec3f(),self.robot_node)
